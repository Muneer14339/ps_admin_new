...
import 'package:pa_sreens/armory/domain/entities/armory_maintenance.dart';
import 'package:pa_sreens/armory/presentation/widgets/add_forms/add_maintenance_form.dart';
      ...
      child: Column(
        ...
        children: [
          ...
          if (widget.firearm != null) ...[
            FirearmItemCard(firearm: widget.firearm!, userId: widget.userId, showDelete: false, title: widget.ammunition!.caliber,),
            const SizedBox(height: 10),
          ],
...
// ADD this method in _ItemDetailsBottomSheetState class
  void _showEditForm() {
    Widget editForm;

    switch (widget.tabType) {
      case ArmoryTabType.firearms:
        editForm = AddFirearmForm(userId: widget.userId, editItem: widget.item as ArmoryFirearm);
        break;
      case ArmoryTabType.ammunition:
        editForm = AddAmmunitionForm(userId: widget.userId, editItem: widget.item as ArmoryAmmunition);
        break;
      case ArmoryTabType.gear:
        editForm = AddGearForm(userId: widget.userId, editItem: widget.item as ArmoryGear);
        break;
      case ArmoryTabType.tools:
        editForm = AddToolForm(userId: widget.userId, editItem: widget.item as ArmoryTool);
        break;
      case ArmoryTabType.maintenence:
        editForm = AddMaintenanceForm(userId: widget.userId, editItem: widget.item as ArmoryMaintenance);
        break;
      case ArmoryTabType.loadouts:
        editForm = AddLoadoutForm(userId: widget.userId, editItem: widget.item as ArmoryLoadout);
        break;
      default:
        return;
    }
    ...
    showDialog(
      context: context,
      builder: (ctx) => DialogWidgets.buildDialogWrapper(
        context: ctx,
        child: editForm,
      ),
    );
