...
class AddAmmunitionForm extends StatefulWidget {
  final String userId;
  final ArmoryAmmunition? editItem;  // ADD

  const AddAmmunitionForm({super.key, required this.userId, this.editItem});
...
class _AddAmmunitionFormState extends State<AddAmmunitionForm> {
  ...
  // REPLACE _populateEditData
  void _populateEditData() {
    final ammo = widget.editItem!;

    // Simple status value
    _dropdownValues['status'] = ammo.status;

    // Text controllers
    _controllers['line']?.text = ammo.line ?? '';
    _controllers['bullet']?.text = ammo.bullet ?? '';
    _controllers['quantity']?.text = ammo.quantity.toString();
    _controllers['lot']?.text = ammo.lot ?? '';
    _controllers['notes']?.text = ammo.notes ?? '';

    // Load initial dropdown data first
    context.read<DropdownBloc>().add(
      const LoadDropdownEvent(
        key: 'ammunition_calibers',
        type: DropdownType.ammunitionCaliber,
    ...
    // Then set values after data loads
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Future.delayed(const Duration(milliseconds: 300), () {
        if (mounted && ammo.caliber.isNotEmpty) {
          final caliberExists = _calibers.any((opt) => opt.value == ammo.caliber);
          _dropdownValues['caliber'] = caliberExists ? ammo.caliber : '__CUSTOM__${ammo.caliber}';

          if (_dropdownValues['caliber'] != null) {
            _loadBrandForCalibers(DialogWidgets.getDisplayValue(_dropdownValues['caliber']!));

            Future.delayed(const Duration(milliseconds: 300), () {
              if (mounted && ammo.brand.isNotEmpty) {
                final brandExists = _ammunitionBrands.any((opt) => opt.value == ammo.brand);
                _dropdownValues['brand'] = brandExists ? ammo.brand : '__CUSTOM__${ammo.brand}';

                if (_dropdownValues['brand'] != null) {
                  _generateBulletTypesForBrand(DialogWidgets.getDisplayValue(_dropdownValues['brand']!));

                  Future.delayed(const Duration(milliseconds: 300), () {
                    if (mounted && ammo.bullet.isNotEmpty) {
                      final bulletExists = _bulletTypes.any((opt) => opt.value == ammo.bullet);
                      _dropdownValues['bulletType'] = bulletExists ? ammo.bullet : '__CUSTOM__${ammo.bullet}';
                      setState(() {});
                    }
                  });
                }
  ...
  void _saveAmmunition() {
    ...
    if (_isEditMode) {
      context.read<ArmoryBloc>().add(UpdateAmmunitionEvent(userId: widget.userId, ammunition: ammunition));
      Navigator.pop(context);

    } else {
      context.read<ArmoryBloc>().add(AddAmmunitionEvent(userId: widget.userId, ammunition: ammunition));
    }
  }
}
