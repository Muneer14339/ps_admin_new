...
class AddLoadoutForm extends StatefulWidget {
  ...
  final ArmoryLoadout? editItem;  // ADD

  const AddLoadoutForm({
    super.key,
    required this.userId,
    this.onNavigateToAddAmmo,
    this.editItem,  // ADD
  });
...
class _AddLoadoutFormState extends State<AddLoadoutForm> {
  ...
  void _populateEditData() {
    final loadout = widget.editItem!;

    _controllers['name']?.text = loadout.name ?? '';
    _controllers['notes']?.text = loadout.notes ?? '';

    _selectedFirearmId = loadout.firearmId;
    _selectedAmmunitionId = loadout.ammunitionId;
    _selectedGearIds = List.from(loadout.gearIds);
    _selectedToolIds = List.from(loadout.toolIds);
      ...
      )
  {
    ...
    if (name.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: const Text('Loadout name is required'),
    ...
    if (_isEditMode) {
      context.read<ArmoryBloc>().add(UpdateLoadoutEvent(userId: widget.userId, loadout: loadout));
      Navigator.pop(context);
    } else {
