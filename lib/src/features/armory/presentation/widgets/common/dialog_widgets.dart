...
import '../../../../../core/utils/caliber_validator.dart';
...
static Widget buildTextField({
    ...
    return Column(
      ...
      children: [
        ...
        DropdownButtonFormField<String>(
          ...
          items: !enabled
              ? [
            // ⭐ CHANGE: Agar disabled hai aur value hai, toh sirf woh value show karo
            if (value != null)
              ...
              : [
            // ⭐ CHANGE: Null option sirf tab add karo jab value null ho
            if (value == null)
              DropdownMenuItem<String>(
                value: null,
                child: Text(
  ...
  static Widget _buildStaticDropdown({
    ...
    // ⭐ FIX: Pehle check karo ki value already options mein hai ya nahi
    final allOptions = List<DropdownOption>.from(options);

    // ⭐ Agar value custom hai aur options mein nahi hai, toh add karo
    if (value != null &&
        isCustomValue(value) &&
    ...
    return Column(
      ...
      children: [
        ...
        DropdownButtonFormField<String>(
          ...
          items: !enabled
              ? [
            // ⭐ Jab disabled hai, sirf selected value show karo
            if (value != null)
              DropdownMenuItem<String>(
                value: value,
                child: Text(
              ...
              : [
            // ⭐ Null option sirf tab add karo jab value null ho
            if (value == null)
              DropdownMenuItem<String>(
                value: null,
                child: Text(
            ...
            // ⭐ Saari options add karo (including custom value jo upar add hui)
            ...allOptions.map(
  ...
  static void _showCustomDialog({
    ...
    showDialog(
      ...
      builder: (dialogContext) => _CustomValueDialog(
        ...
        validator: label.toLowerCase() == 'caliber'
            ? (value) => CaliberValidator.validate(value)  // ✅ ADD - Caliber ke liye specific validator
            : validator,
      ),
    );
  }
...
class _SearchableDropdownFieldState extends State<_SearchableDropdownField> {
  ...
  final LayerLink _layerLink = LayerLink();  // ADD
  List<DropdownOption> _filteredOptions = [];
  bool _showDropdown = false;
  String? _displayText;
  OverlayEntry? _overlayEntry;  // ADD
  ...
  void _updateDisplayText() {
    if (widget.value != null && widget.value!.isNotEmpty) {
      _displayText = DialogWidgets.getDisplayValue(widget.value);
    } else {
      _displayText = null;
    }
  }
  ...
  @override
  void dispose() {
    _overlayEntry?.remove();  // ADD
    _DropdownStateManager.closeDropdown(this);
  ...
  void closeDropdown() {
    if (mounted) {
      _overlayEntry?.remove();  // ADD
      _overlayEntry = null;  // ADD
      setState(() => _showDropdown = false);
      _focusNode.unfocus();
    }
  }

  void _showDropdownOverlay() {  // ADD NEW METHOD
        ...
        child: CompositedTransformFollower(
          ...
          child: Material(
            ...
            child: Container(
              ...
              child: TapRegion(  // ADD - prevents closing when tapping inside dropdown
  ...
  Widget build(BuildContext context) {
    return CompositedTransformTarget(  // WRAP Column with this
      link: _layerLink,
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          TextFormField(
            ...
            decoration: InputDecoration(
              hintText: widget.enabled
                  ? (widget.isLoading
                  ? 'Loading...'
                  : _displayText ?? 'Search ${widget.label.replaceAll('*', '').trim().toLowerCase()}...')
                  : 'Select required field first...',
              filled: true,
              fillColor: AppTheme.background(context),
              border: OutlineInputBorder(
...
// Global dropdown state manager (ADD before _SearchableDropdownField class)
class _DropdownStateManager {
  static _SearchableDropdownFieldState? _currentlyOpenDropdown;

  static void openDropdown(_SearchableDropdownFieldState dropdown) {
    _currentlyOpenDropdown?.closeDropdown();
    _currentlyOpenDropdown = dropdown;
  }

  static void closeDropdown(_SearchableDropdownFieldState dropdown) {
    if (_currentlyOpenDropdown == dropdown) {
