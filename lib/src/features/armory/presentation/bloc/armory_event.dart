...
// ADD these events after existing delete events
class UpdateFirearmEvent extends ArmoryEvent {
  final String userId;
  final ArmoryFirearm firearm;
  const UpdateFirearmEvent({required this.userId, required this.firearm});

  @override
  List<Object?> get props => [userId, firearm];
}

class UpdateAmmunitionEvent extends ArmoryEvent {
  final String userId;
  final ArmoryAmmunition ammunition;
  const UpdateAmmunitionEvent({required this.userId, required this.ammunition});

  @override
  List<Object?> get props => [userId, ammunition];
}

class UpdateGearEvent extends ArmoryEvent {
  final String userId;
  final ArmoryGear gear;
  const UpdateGearEvent({required this.userId, required this.gear});

  @override
  List<Object?> get props => [userId, gear];
}

class UpdateToolEvent extends ArmoryEvent {
  final String userId;
  final ArmoryTool tool;
  const UpdateToolEvent({required this.userId, required this.tool});

  @override
  List<Object?> get props => [userId, tool];
}

class UpdateLoadoutEvent extends ArmoryEvent {
  final String userId;
  final ArmoryLoadout loadout;
  const UpdateLoadoutEvent({required this.userId, required this.loadout});

  @override
  List<Object?> get props => [userId, loadout];
}

class UpdateFirearmWithDependenciesEvent extends ArmoryEvent {
  final String userId;
  final ArmoryFirearm firearm;
  final List<String> removedCalibers;
  final List<String> dependentLoadoutIds;
  final List<String> dependentAmmunitionIds;

  const UpdateFirearmWithDependenciesEvent({
    required this.userId,
    required this.firearm,
    required this.removedCalibers,
    required this.dependentLoadoutIds,
    required this.dependentAmmunitionIds,
  });

  @override
  List<Object?> get props => [userId, firearm, removedCalibers, dependentLoadoutIds, dependentAmmunitionIds];
}

class UpdateMaintenanceEvent extends ArmoryEvent {
  final String userId;
  final ArmoryMaintenance maintenance;

  const UpdateMaintenanceEvent({required this.userId, required this.maintenance});
