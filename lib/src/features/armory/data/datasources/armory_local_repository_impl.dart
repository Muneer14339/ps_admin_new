...
import 'package:pa_sreens/src/features/armory/data/models/armory_ammunition_model.dart';
import 'package:pa_sreens/src/features/armory/data/models/armory_firearm_model.dart';
import 'package:pa_sreens/src/features/armory/data/models/armory_gear_model.dart';
import 'package:pa_sreens/src/features/armory/data/models/armory_loadout_model.dart';
import 'package:pa_sreens/src/features/armory/data/models/armory_maintenance_model.dart';
import 'package:pa_sreens/src/features/armory/data/models/armory_tool_model.dart';
...
// MODIFY updateAmmunition
  ...
  // Get items by ID for session history
  @override
  Future<ArmoryLoadoutModel?> getLoadoutById(String userId, String loadoutId) async {
    try {
      print('getLoadoutById - userId: $userId, loadoutId: $loadoutId');
      final db = await this.db;
      final maps = await db.query('loadouts',
          where: 'userId = ? AND id = ? AND syncStatus != ?',
          whereArgs: [userId, loadoutId, 'deleted'],
          limit: 1
      );
      print('getLoadoutById - query result count: ${maps.length}');
      if (maps.isNotEmpty) {
        print('getLoadoutById - found loadout: ${maps.first}');
      }
      if (maps.isEmpty) return null;
      return ArmoryLoadoutModel.fromMap(maps.first, maps.first['id'] as String);
    } catch (e) {
      log('Error getting loadout by ID: $e');
      print('getLoadoutById - ERROR: $e');
      return null;
    }
  }

  @override
  Future<ArmoryFirearmModel?> getFirearmById(String userId, String firearmId) async {
    try {
      print('getFirearmById - userId: $userId, firearmId: $firearmId');
      final db = await this.db;
      final maps = await db.query('firearms',
          where: 'userId = ? AND id = ? AND syncStatus != ?',
          whereArgs: [userId, firearmId, 'deleted'],
      ...
      print('getFirearmById - query result count: ${maps.length}');
      if (maps.isNotEmpty) {
        print('getFirearmById - found firearm: ${maps.first}');
      }
      if (maps.isEmpty) return null;
      return ArmoryFirearmModel.fromMap(maps.first, maps.first['id'] as String);
    } catch (e) {
      log('Error getting firearm by ID: $e');
      print('getFirearmById - ERROR: $e');
      return null;
    }
  }

  @override
  Future<ArmoryAmmunitionModel?> getAmmunitionById(String userId, String ammunitionId) async {
    try {
      print('getAmmunitionById - userId: $userId, ammunitionId: $ammunitionId');
      final db = await this.db;
      final maps = await db.query('ammunition',
          where: 'userId = ? AND id = ? AND syncStatus != ?',
          whereArgs: [userId, ammunitionId, 'deleted'],
      ...
      print('getAmmunitionById - query result count: ${maps.length}');
      if (maps.isNotEmpty) {
        print('getAmmunitionById - found ammunition: ${maps.first}');
      }
      if (maps.isEmpty) return null;
      return ArmoryAmmunitionModel.fromMap(maps.first, maps.first['id'] as String);
    } catch (e) {
      log('Error getting ammunition by ID: $e');
      print('getAmmunitionById - ERROR: $e');
      return null;
    }
  }
}