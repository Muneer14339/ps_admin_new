...
//    platform_log("BiDirectionVerification: Starting with fit_num=%d", fitComb.usesize());
    ...
    double max_score(-1);
    cv::RotatedRect max_ellipse;
    // EMERGENCY ABORT FOR 6MM LENS
    if (fit_num > 500) {
//        platform_log("BiDirectionVerification: EMERGENCY ABORT - fit_num=%d too large, skipping frame", fit_num);
        return;
    }
...
//                platform_log("BiDirectionVerification: TIMEOUT after %dms at iteration %d/%d - aborting",
//                             (int)duration.count(), i, fit_num);
                return;
            }

//            if (i > 0) {
//                platform_log("BiDirectionVerification: iteration %d/%d, elapsed %dms", i, fit_num, (int)duration.count());
//            }
        }
...
//            // LOG IF WE HAVE LARGE NESTED LOOPS
//            if (linked_num > 50 || linking_num > 50) {
//                platform_log("BiDirectionVerification: LARGE LOOPS detected - linked_num=%d, linking_num=%d", linked_num, linking_num);
//            }
...
//            auto loop_duration = std::chrono::duration_cast<std::chrono::milliseconds>(loop_end - loop_start);
            ...
            // LOG IF NESTED LOOPS TOOK TOO LONG
//            if (loop_duration.count() > 10) {
//                platform_log("BiDirectionVerification: SLOW NESTED LOOPS - took %dms for linked_num=%d, linking_num=%d",
//                             (int)loop_duration.count(), linked_num, linking_num);
//            }

            if (isCombValid == false) // 两组弧段不能组合
                continue;
            fitres = FittingConstraint(fitMats[0][idx_r].val, fitMats[1][idx_l].val, fitelpres);
        }
        else if (idx_l >= 0)
...
//        if (validation_duration.count() > 20) {
//            platform_log("BiDirectionVerification: SLOW VALIDATION - took %dms", (int)validation_duration.count());
//        }
        if (fitres == false)
            continue;
        num_idx_l = idx_l >= 0 ? search_group[1][idx_l].size() : -1;
...
//    platform_log("BiDirectionVerification: COMPLETED in %dms, processed %d combinations", (int)total_duration.count(), fit_num);
}
