...
//
//	// Filter ellipses to keep only the outer boundary of each ring
...
//
//	// Function to estimate minimum distance between two ellipse boundaries
//	auto minBoundaryDistance = [](const cv::RotatedRect& e1, const cv::RotatedRect& e2) -> float {
//		const int num_points = 20; // Sample points along each ellipse
//		float min_dist = std::numeric_limits<float>::max();
//
//		// Ellipse 1 parameters (use original coordinates, no swapping)
//		cv::Point2f c1 = e1.center;
//		float a1 = e1.size.width / 2.0f;
//		float b1 = e1.size.height / 2.0f;
//		float theta1 = e1.angle * CV_PI / 180.0f;
//		float cos_theta1 = cos(theta1), sin_theta1 = sin(theta1);
//
//		// Ellipse 2 parameters
//		cv::Point2f c2 = e2.center;
//		float a2 = e2.size.width / 2.0f;
//		float b2 = e2.size.height / 2.0f;
//		float theta2 = e2.angle * CV_PI / 180.0f;
//		float cos_theta2 = cos(theta2), sin_theta2 = sin(theta2);
//
//		// Sample points on ellipse 1
...
//
//			// Sample points on ellipse 2
//			for (int j = 0; j < num_points; ++j) {
//				float u = j * 2 * CV_PI / num_points;
//				float x2 = a2 * cos(u);
//				float y2 = b2 * sin(u);
...
//	};
//
//	// Group ellipses by boundary proximity (distance < 20 pixels)
...
//			float dist = minBoundaryDistance(filtered_ellipses[i], filtered_ellipses[j]);
//			if (dist < 20.0f) { // Very close boundaries (20 pixels)
//				group.push_back(j);
//				grouped[j] = true;
//			}
//		}
...
//	}
//
//	// For each group, keep only the outer boundary (by average size)
//	for (const auto& group : groups) {
//		if (group.size() <= 1) continue; // Keep isolated ellipses
//		size_t largest_idx = group[0];
//		float max_size = (filtered_ellipses[largest_idx].size.width + filtered_ellipses[largest_idx].size.height) / 2.0f;
...
//			}
//		}
//		// Mark all but the largest ellipse as not kept
//		for (size_t idx : group) {
//			if (idx != largest_idx) {
//				keep[idx] = false;
//			}
...
//
//
//	// Filter ellipses to keep outer boundary
//	std::vector<cv::RotatedRect> filtered_ellipses = pre_nms_ellipses;
//	std::vector<double> filtered_scores = pre_nms_scores;
//	std::vector<bool> keep(filtered_ellipses.size(), true);
//
...
//	Point vecFeature[8];
//
//	LinkMatrix.Update(fsaarcs_num*fsaarcs_num);
//	char* _LinkMatrix = LinkMatrix.GetDataPoint();
//
//	int T_ij, T_ji;
//	Point *_vec_data_i = NULL, *_vec_data_j = NULL;
//
//
//	// 1-25 Add
...
//			findIdx = _indices_arcs[j];
//			_linkIdx = i*fsaarcs_num + findIdx;
//			if (_LinkMatrix[_linkIdx] != 0)
//				continue;
//			int findArcNum = fsaarcs[findIdx].size();
//			_vec_data_j = fsaarcs[findIdx].data();
//
...
//			switch (distType)
//			{
//			case FLED_GROUPING_FBmA1_FAnB1:
//			{
...
//				break;
//			}
//			case FLED_GROUPING_FBmA1_CAnB1:
//			{
//				group_res = Group4CAnB1_FBmA1(vecFeature, l1, l2);
//				//case_stat[1]++;
//				break;
//			}
//			case FLED_GROUPING_CBmA1_FAnB1:
//			{
//				group_res = Group4FAnB1_CBmA1(vecFeature, l1, l2);
//				//case_stat[2]++;
//				break;
//			}
//			case FLED_GROUPING_CBmA1_CAnB1:
//			{
//				if (findIdx == i)
//				{
//					group_res = 1;
...
//			if (_linkMatrix[idx + j] == 1)
//				lA[i].idx_linking.push_back(j);
//			else if (_linkMatrix[idx + j] == -1)
...
bool FLED::isValidFrame(const cv::Mat &Img_G, std::string &message) {
    ...
    for (int i = 0; i < hist_size; ++i) {
        if (hist.at<float>(i) > 0.03) peak_count++;
    }

    // Heuristic for initial valid frames
    bool is_valid = false;
    if (hfe > 50.0 && peak_count >= 2 && hist.at<float>(0) < 0.9 && hist.at<float>(15) < 0.9) {
        is_valid = true;
    }
    ...
    double min_hfe = (valid_frame_count >= 3) ? baseline_hfe * 0.3 : 50.0;
    double min_peaks = (valid_frame_count >= 3) ? baseline_peaks * 0.5 : 1.0;
    // Validation checks
    if (hfe < min_hfe) {
