...
//    }
//
//    // NEW: Store first frame for 500ms comparison
...
//    // Quick checks - only flag if there's a problem
...
//
//
//    // ==================== NEW: 500MS FRAME COMPARISON LOGIC ====================
//    if (!first_comparison_frame.empty() &&
//        (timestamp - first_comparison_timestamp >= 200) &&
//        (timestamp - last_hit_time >= 200)) {
//
...
//        }
//
//        // Step 5: Check motion pixel count
////        int comp_total_motion_pixels = cv::countNonZero(comp_crop_motion);
////        if (comp_total_motion_pixels < 15) {
////            platform_log("[500MS_MOTION] Too little motion: %d pixels", comp_total_motion_pixels);
////            first_comparison_frame = frame_gray.clone();
////            first_comparison_timestamp = timestamp;
////            return -1;
////        }
//
//        // Step 6: Apply black mask and new_bullet_mask AND operations
//        cv::Mat comp_joint_crop_motion;
//        cv::bitwise_and(comp_crop_motion, comp_crop_black_mask, comp_joint_crop_motion);
////        cv::bitwise_and(comp_joint_crop_motion, comp_new_bullet_mask, comp_joint_crop_motion);
//
//        // Step 7: Consistency check
////        cv::Mat comp_motion_and_dark;
////        cv::bitwise_and(comp_crop_motion, comp_new_bullet_mask, comp_motion_and_dark);
////        int comp_consistent_pixels = cv::countNonZero(comp_motion_and_dark);
////        if (comp_total_motion_pixels > 25 && comp_consistent_pixels == 0) {
////            platform_log("[500MS_CONSISTENCY] Large motion but no dark pixels");
////            first_comparison_frame = frame_gray.clone();
////            first_comparison_timestamp = timestamp;
////            return -1;
////        }
...
//
//        // Step 9: Check if motion exists but no valid contours
////        if (comp_origin_contours.size() > 0 && comp_contours.empty()) {
////            platform_log("[500MS_BULLET_MISS] Motion detected but no valid contours");
////            first_comparison_frame = frame_gray.clone();
////            first_comparison_timestamp = timestamp;
////            return -1;
////        }
...
//            // Step 10: Sort contours by area
//            std::vector<std::vector<cv::Point>> comp_sorted_origin = comp_origin_contours;
//            std::sort(comp_sorted_origin.begin(), comp_sorted_origin.end(),
//                      [](const std::vector<cv::Point> &a, const std::vector<cv::Point> &b) {
//                          return cv::contourArea(a) > cv::contourArea(b);
//                      });
//            comp_sorted_origin.resize(std::min(comp_sorted_origin.size(), size_t(5)));
//
...
//
//            // Step 11: Check area constraints
...
//            }
//
//            // Step 12: Check radius constraints
//            cv::Point2f comp_center;
//            float comp_radius;
//            cv::minEnclosingCircle(comp_bullet_contour, comp_center, comp_radius);
//            platform_log("[500MS_BULLET] Best contour: area=%.1f, radius=%.1f", comp_area, comp_radius);
//
...
//            }
//
//            // Step 13: Calculate bullet radius
...
//                platform_log("[500MS_AREA] Invalid bullet_radius: %.1f", comp_bullet_radius);
//                first_comparison_frame = frame_gray.clone();
//                first_comparison_timestamp = timestamp;
//                return -1;
//            }
//
//            // Step 14: Apply strict checks
////            bool comp_strict_checks = (comp_area >= 1 && comp_area <= 500) &&
////                                      (comp_bullet_radius >= 0.1 && comp_bullet_radius <= 10.0);
////
////            if (is_ble && comp_strict_checks) {
//                platform_log("[500MS_AREA] valid area: %.1f", comp_area);
//
//                // Step 15: Add to frame buffer
...
//                }
//
//                // Step 16: Add to history
////                history.push_back(comp_bullet_contour);
////                last_hit_point = comp_center;
//                last_hit_time = timestamp;
//
//                // Step 17: Majority voting for score
//                std::vector<int> comp_scores;
//                for (size_t i = 0; i < frame_buffer.size(); ++i) {
//                    const auto &data = frame_buffer[i];
//                    int score = calculate_score(comp_bullet_contour, data.ellipses);
//                    comp_scores.push_back(score);
//                }
//
//                std::map<int, int> comp_score_counts;
//                for (int score : comp_scores) {
//                    comp_score_counts[score]++;
//                }
//                int comp_assigned_score = 0;
//                int comp_max_count = 0;
//                for (const auto &pair : comp_score_counts) {
...
//                }
//
//                if (comp_max_count <= 1 && !comp_scores.empty()) {
//                    comp_assigned_score = comp_scores.back();
//                }
//
//                // Step 18: Calculate bullet center and direction
...
//
//                // Step 19: Update all tracking variables
...
//                history.push_back(comp_bullet_contour);
//                last_hit_point = comp_bullet_center;
//
//                // Step 20: Reset comparison frame
...
//            platform_log("[SCORE_RETURNED] From 500MS LOGIC: score=%d, time=%ldμs", comp_assigned_score, frame_duration.count());
...
//
//            int total_motion_pixels = cv::countNonZero(crop_motion);
//            if (total_motion_pixels < 15) {
//                platform_log("[MOTION] Too little motion: %d pixels", total_motion_pixels);
//                return -1;
//            }
//
//            cv::Mat motion_and_dark;
//            cv::bitwise_and(crop_motion, new_bullet_mask, motion_and_dark);
//            int consistent_pixels = cv::countNonZero(motion_and_dark);
//            if (total_motion_pixels > 25 && consistent_pixels == 0) {
//                platform_log("[CONSISTENCY] Large motion but no dark pixels");
////            return -1;
//            }
//
//            platform_log("[DEBUG] Large motion detected: %d pixels", total_motion_pixels);
//
//            std::vector<std::vector<cv::Point>> sorted_origin_contours = origin_contours;
//            std::sort(sorted_origin_contours.begin(), sorted_origin_contours.end(),
//                      [](const std::vector<cv::Point> &a, const std::vector<cv::Point> &b) {
//                          return cv::contourArea(a) > cv::contourArea(b);
//                      });
...
//            if (area < 5 || area > 1000) {
//                platform_log("[AREA] Invalid area: %d", area);
//                return -1;
//            }
//
//            cv::Point2f center;
...
//            platform_log("[BULLET] Best contour: area=%.1f, radius=%.1f", area, radius);
//
...
//            }
//
//            if (bullet_radius < 1.0) {
//                platform_log("[AREA] valid area: %d", bullet_radius);
//                return -1;
//            }
//
//            bool strict_checks = (area >= 1 && area <= 500) &&
//                                 (bullet_radius >= 0.1 && bullet_radius <= 10.0);
//
//            if (is_ble && strict_checks) {
//                platform_log("[AREA] valid area: %d", area);
//
//                FrameData frame_data;
//                frame_data.frame = frame.clone();
//                frame_data.ellipses = ellipses;
//                frame_data.bullet_contour = bullet_contour;
...
//                for (size_t i = 0; i < frame_buffer.size(); ++i) {
//                    const auto &data = frame_buffer[i];
//                    int score = calculate_score(bullet_contour, data.ellipses);
...
//                        assigned_score = pair.first;
//                        max_count = pair.second;
//                    }
//                }
//
//                if (max_count <= 1 && !scores.empty()) {
//                    assigned_score = scores.back();
//                }
//
//                cv::Moments moments = cv::moments(bullet_contour);
//                cv::Point2f bullet_center(moments.m10 / moments.m00, moments.m01 / moments.m00);
//
//                int direction = getBulletDirection(bullet_center,
//                                                   cv::Point2f(target_center.x, target_center.y));
//
...
//
//                // Reset comparison frame after successful score
//                first_comparison_frame.release();
//                first_comparison_timestamp = 0;
//
////                auto frame_end_time = std::chrono::high_resolution_clock::now();
////                auto frame_duration = std::chrono::duration_cast<std::chrono::microseconds>(frame_end_time - frame_start_time);
////                platform_log("[SCORE_RETURNED] From PREVIOUS LOGIC: score=%d, time=%ldμs", assigned_score, frame_duration.count());
//                return assigned_score;
//
//            } else {
//                return -1;
...
//    // ==================== NEW: 500MS FRAME COMPARISON LOGIC ====================
//    if (!first_comparison_frame.empty() &&
//        (timestamp - first_comparison_timestamp >= 500) &&
//        (timestamp - last_hit_time >= 500)) {
//
//        // Step 1: Calculate motion between stored frame and current frame
//        cv::Mat comparison_blur_first, comparison_blur_current;
//        cv::GaussianBlur(first_comparison_frame, comparison_blur_first, cv::Size(11, 11), 0);
...
//
//
////        if (!comp_contours.empty() && time_cond) {
//        if (!comp_contours.empty()) {
//            // Step 10: Sort contours by area
//            std::vector <std::vector<cv::Point>> comp_sorted_origin = comp_origin_contours;
//            std::sort(comp_sorted_origin.begin(), comp_sorted_origin.end(),
...
//            float comp_radius;
//            cv::minEnclosingCircle(comp_bullet_contour, comp_center, comp_radius);
//            platform_log("[500MS_BULLET] Best contour: area=%.1f, radius=%.1f", comp_area,
//                         comp_radius);
//
//            if (comp_radius < 1.0 || comp_radius > 15.0) {
////                first_comparison_frame = frame_gray.clone();
...
//
//
////            bool strict_checks = (comp_area >= 1 && comp_area <= 500) &&
////                                 (comp_bullet_radius >= 0.1 && comp_bullet_radius <= 10.0);
//
////            if (is_ble && strict_checks) {
//                platform_log("[500MS_AREA] valid area: %.1f", comp_area);
...
//                    comp_assigned_score = comp_scores.back();
//                }
//                if (comp_assigned_score == 0) {
//                    platform_log("[500MS] Score is 0, ignoring shot");
////                    first_comparison_frame.release();
//                    first_comparison_timestamp = timestamp;
//
//                    return -1;
...
///Old_2_dec
/// Add these member variables to your Scorer class header
...
//
//// Modified process function with 3-frame rolling window
//int Scorer::process(
//        cv::Mat frame,
//        cv::Mat frame_gray,
//        std::map <std::string, std::vector<cv::RotatedRect>> target_zone,
...
//
//            if (comp_radius < 1.0 || comp_radius > 15.0) {
//                platform_log("[500MS_AREA] Invalid comp_radius: %.1f", comp_radius);
//
//                first_comparison_timestamp = 0;
//
//                first_comparison_timestamp = timestamp;
...
//                                           comp_moments.m01 / comp_moments.m00);
//
//            // Calculate score for this detection
...
//
//            // Check if this detection is too close to a recently confirmed hit
////            bool too_close_to_confirmed = false;
////            for (const auto& confirmed_hit : confirmed_hits) {
////                float distance = cv::norm(confirmed_hit.position - comp_bullet_center);
////                if (distance < POSITION_TOLERANCE) {
////                    too_close_to_confirmed = true;
////                    platform_log("[3-FRAME] Detection too close to confirmed hit (%.1fpx) - skipping", distance);
////                    break;
////                }
////            }
////
////            // If too close to already confirmed hit, skip and reset
////            if (too_close_to_confirmed) {
//////                first_comparison_frame = frame_gray.clone();
////                first_comparison_timestamp = timestamp;
////                return -1;
////            }
