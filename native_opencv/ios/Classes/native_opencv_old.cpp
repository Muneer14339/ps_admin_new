...
//        for (const auto &contour : history)
//        {
//            cv::Point2f center;
//            float radius;
//            cv::minEnclosingCircle(contour, center, radius);
//            cv::circle(frame, center, radius - 4, cv::Scalar(0, 0, 255), 2); // All history in red
//        }
//        // For the most recent shot, highlight it in yellow-green
//        if (!history.empty())
//        {
//            cv::Point2f center;
//            float radius;
//            cv::minEnclosingCircle(history.back(), center, radius);
//            cv::circle(frame, center, radius - 4, cv::Scalar(222, 255, 0), 2); // Latest in yellow-green
//        }
//
...
//        if (area > 3000)
//        {
//            // std::cout << "[DEBUG] Contour area too large (" << area << "), rejecting as noise" << std::endl;
//            return -1;
//        }
//        cv::Point2f center;
//        float radius;
...
//        if (radius < 1.5)
//        {
//            // std::cout << "[DEBUG] Initial radius too small (" << radius << "), early exit for noise" << std::endl;
//            return -1;
//        }
//        // Polygon test with area limit to avoid large contour replacement
//        bool replaced = false;
//        for (const auto &contour : sorted_origin_contours)
//        {
//            double contour_area = cv::contourArea(contour);
//            double result = cv::pointPolygonTest(contour, center, false);
//            // std::cout << "[SHOT DEBUG] Polygon test for origin contour " << result << std::endl;
//            if (result > 0 && contour_area < area * 1.5) // only replace if contour area close to original
//            {
...
//            }
//        }
//        // std::cout << "[DEBUG] x1: " << x1 << ", y1: " << y1 << std::endl;
//        // std::cout << "[DEBUG] Bullet center before shift: " << center.x << ", " << center.y << std::endl;
...
//            point.y += y1;
//        }
//        // NOW: Use robust bullet radius calculation for final validation
//        double contour_area = cv::contourArea(bullet_contour);
//        float radius_area = std::sqrt(contour_area / CV_PI);
//        float radius_enclose;
//        cv::minEnclosingCircle(bullet_contour, center, radius_enclose);
//        // std::cout << "[DEBUG] Bullet center after shift: " << center.x << ", " << center.y << std::endl;
//        //  Choose best radius
//        float bullet_radius;
//        if (radius_area > radius_enclose * 1.1)
//        {
//            bullet_radius = radius_enclose;
//        }
//        else if (radius_enclose > radius_area * 1.1)
//        {
//            bullet_radius = radius_area;
//        }
//        else
//        {
//            bullet_radius = (radius_enclose + radius_area) / 2.0f;
//        }
//        // Final plausibility check (you can tune the value, e.g. 5mm for 0.22 bullet)
//        if (bullet_radius < 1.5)
//        {
//            // std::cout << "Bullet radius too small (" << bullet_radius << "), skipping as noise." << std::endl;
//            return -1;
//        }
//        // Sort circles by radius ascending (innermost first)
...
//        // ---- Scoring with verbose output ----
//        int highest_score = 10;
//        int lowest_score = 5;
//        int n_rings = circles.size();
//        bool found = false;
//        // ---- CORRECTED EDGE-BASED Scoring ----
//        int assigned_score = 0;
//        cv::Point bullseye_center = circles[0].first; // Use first ring center as reference
//        float center_distance = cv::norm(center - cv::Point2f(bullseye_center));
//        // CRITICAL: Use bullet edge distance, not center distance
//        float bullet_edge_distance = center_distance - bullet_radius;
//        float tolerance = 3.0f;
//        for (int i = 0; i < circles.size(); ++i)
//        {
//            float ring_radius = circles[i].second;
//            float distance_to_ring = bullet_edge_distance - ring_radius;
//            std::cout << "Ring " << i << ": radius=" << ring_radius
//                      << "px, bullet_edge_distance=" << bullet_edge_distance
//                      << "px, distance_to_ring=" << distance_to_ring << "px";
//            if (distance_to_ring <= tolerance)
//            {
//                assigned_score = 10 - i;
//                std::cout << ", assigned_score=" << assigned_score << std::endl;
//                break;
//            }
//            else
//            {
//                std::cout << ", no score assigned" << std::endl;
//            }
//        }
//        if (assigned_score == 0)
//        {
//            std::cout << "Bullet missed all rings. Assigned score=0" << std::endl;
//        }
//        // Update scoring variables
//        last_score = assigned_score;
//        total_score += assigned_score;
//        score_history.push_back(assigned_score);
...
//        //
//        //
//        // auto circles = target_zone["circles"];
//        // std::sort(circles.begin(), circles.end(),
//        //
//        //[](const std::pair<cv::Point, int>& a, const std::pair<cv::Point, int>& b) {
//        // return a.second < b.second;
//        //});
//        //
//        //// Calculate center-to-bullseye distance
//        // float center_distance = cv::norm(center - roi_center);
//        // float bullet_radius   = radius;  // from your minEnclosingCircle
//        //
//        // platform_log("Bullet center: (%.1f, %.1f)\n", center.x, center.y);
//        // platform_log("Bullet radius: %.3f\n", radius);
//        // platform_log("Bullseye center: (%.1f, %.1f)\n", roi_center.x, roi_center.y);
//        // platform_log("center_distance: %.3f\n", center_distance);
//        //
//        // for (size_t i = 0; i < circles.size(); ++i) {
//        // platform_log("Ring %zu radius: %d\n", i, circles[i].second);
//        // }
//        //
//        //// Initialize assigned score to 0 (outside all rings)
//        // int assigned_score = 0;
//        //
//        //// Calculate bullet's inner edge from target center
//        // float bullet_inner_edge = center_distance - radius;
//        //
//        // for (size_t i = 0; i < circles.size(); ++i) {
//        // float ring_radius = static_cast<float>(circles[i].second);
//        // float dist_center_to_line = center_distance - ring_radius;
//        //
//        // float separation = std::fabs(dist_center_to_line) - bullet_radius;
//        // bool  touches   = (separation <= 0.0f);
//        //
//        // platform_log(
//        //"Ring %zu | ring_radius=%.3f | center_dist=%.3f | dist_to_line=%.3f | "
//        //"radius=%.3f | separation=%.3f | touches? %s\n",
//        // i,
//        // ring_radius,
//        // center_distance,
//        // dist_center_to_line,
...
//        //
//        // platform_log(
//        //"Checking ring %zu | ring_radius: %.3f | bullet_inner_edge: %.3f | center_distance: %.3f\n",
//        // i, ring_radius, bullet_inner_edge, center_distance
//        //);
//        //
//        //// require inner edge to be at least 2px inside
//        // if (bullet_inner_edge <= ring_radius - 1.6f) {
//        //
//        // assigned_score = 10 - static_cast<int>(i);
//        // platform_log(
//        //"  -> inner edge is inside ring %zu by %.3f px. Assigned score: %d\n",
//        // i, (ring_radius - bullet_inner_edge), assigned_score
//        //);
//        // break;
//        // }
//        // }
//        //
//        //// Apply outer ring cutoff rule
//        // if (assigned_score < 5 && assigned_score != 0) {
//        // assigned_score = 0;
//        // }
//        //
//        // platform_log("Final assigned score for this bullet: %d\n", assigned_score);
//        //
//        //// Update scoring history and return
//        // last_score = assigned_score;
//        // total_score += assigned_score;
//        // score_history.push_back(assigned_score);
//        // return assigned_score;
...
//        //
//        //
//        ////// Sort rings from smallest to largest (innermost to outermost)
//        ////auto circles = target_zone["circles"];
//        ////std::sort(circles.begin(), circles.end(),
//        ////[](const std::pair<cv::Point, int>& a, const std::pair<cv::Point, int>& b) {
//        ////return a.second < b.second;
//        ////});
//        ////// The number of scoring rings (should be 6 for 10-5 scoring)
//        ////int num_rings = int(circles.size());
//        ////int assigned_score = 0; // Default: miss
//        ////// The highest score (10) is for the innermost ring
//        ////// Compute bullet's outer edge distance from bullseye
//        ////float bullet_edge = min_distance_to_bullseye + radius;
//        ////// Loop from innermost (score 10) to outermost (score 5)
//        ////for (int i = 0; i < num_rings; ++i) {
//        ////float ring_radius = float(circles[i].second);
//        ////if (bullet_edge <= ring_radius) {
//        ////assigned_score = 10 - i; // 10, 9, ..., 5
//        ////break;
//        ////}
//        ////}
//        ////// If bullet_edge is greater than outermost ring, assigned_score stays 0
//        ////// Clamp to [5,10] (if needed)
//        ////if (assigned_score < 5 && assigned_score != 0)
//        ////assigned_score = 0; // only 5 to 10 are valid, else 0 for miss
//        ////last_score = assigned_score;
//        ////total_score += assigned_score;
//        ////score_history.push_back(assigned_score);
//        ////return assigned_score;
...
//                                    cv::circle(frame, circle.first, circle.second, cv::Scalar(255, 0, 0), 0);
//                                }
//                                else
//                                {
...
//            std::string text = "#framen " + std::to_string(frame_index) + " fps: " + std::to_string(runningFps);
...
//    }
//
//    /// Circle Detection and scoring is without circle overlay
//    // FUNCTION_ATTRIBUTE
//    //         int32_t score(uint8_t *input_buffer, int platform, int _, int frame_width, int frame_height,
//    //         uint8_t *output_buffer, uint8_t *detect_only_buffer, char *output_path) {
//    // long long start = get_now();
...
//    // int32_t frame_index = 0;
//    // int32_t last_score_out = -1;
//    // int32_t detect_only = 1;  // 1 = detect-only mode, 0 = scoring mode
//    //
//    // int max_size = 1024;
//    // platform_log("Frame_C width: %d frame height: %d", frame_width, frame_height);
//    //
...
//    // sleep_ms(100);
//    //
//    //// --- Read frame index & fps from input buffer ---
//    // input_mtx.lock();
//    // input_buffer = input_buffer_;
//    // frame_index = readInt32FromBuffer(input_buffer);
//    // if (frame_index < 0 || frame_index == prev_frame_index) {
...
//    //
//    // fps = readInt32FromBuffer(input_buffer);
//    // input_buffer += sizeof(int32_t) * 2; // skip last_score & num_bytes fields
//    //
//    // int n_bytes = readInt32FromBuffer(input_buffer);
//    // input_buffer += sizeof(int32_t);
//    //
...
//    // }
//    //
//    //// --- Crop / Resize to match expected frame_width/frame_height ---
...
//    // }
//    //
//    //// --- Detection & (optional) scoring ---
//    // if (!frame.empty() && !frame_gray.empty()) {
//    // int numCircles = tz_detector.process(frame, frame_gray);
//    // if (numCircles > 0 && !tz_detector.is_shifted()) {
//    // auto target_zone = tz_detector.get_target_zone_info();
//    // if (target_zone.count("circles")) {
//    // auto circles = target_zone["circles"];
//    // circleCount = int(circles.size());
//    //
//    //// read detect_only flag
//    // detect_only_mtx.lock();
//    // detect_only = readInt32FromBuffer(detect_only_buffer_);
//    // detect_only_mtx.unlock();
//    //
//    // if (detect_only) {
//    //// --- DETECT-ONLY: draw circles, no score ---
//    // for (auto &c : circles) {
//    // const cv::Point &center = c.first;
//    // int radius = c.second;
//    // if (radius > 0) {
//    // cv::circle(frame, center, radius, cv::Scalar(0, 255, 0), 0);
//    // }
//    // }
//    // } else {
//    //// --- SCORING MODE: compute & draw score, no circles ---
//    // float timestamp = fps > 0
//    //                   ? static_cast<float>(frame_index) / fps
//    //                   : 0.f;
//    // last_score_out = scorer.process(
//    //         frame, frame_gray, target_zone, timestamp);
//    //
//    // std::string scoreText = "Score: " + std::to_string(last_score_out);
//    // cv::putText(
//    //         frame, scoreText,
//    //         cv::Point(50, 80),
//    //         cv::FONT_HERSHEY_SIMPLEX,
//    // 0.8, cv::Scalar(0, 255, 0), 5, cv::LINE_AA
//    //);
//    // }
//    // } else {
//    // platform_log("Target zone missing 'circles' key.");
...
//    // double t1 = static_cast<double>(cv::getTickCount());
//    // int runningFps = cvRound(cv::getTickFrequency() / (t1 - t0));
//    // std::string fpsText = "#frame " + std::to_string(frame_index)
...
//    //);
//    //
//    //// restore to original size if scaled
//    // if (ratio < 1) {
//    // cv::resize(frame, frame, cv::Size(frame_width, frame_height));
//    // }
//    //
//    //// --- Write outputs back into output_buffer_ ---
//    // mtx.lock();
//    // output_buffer = output_buffer_;
//    // std::memcpy(output_buffer, &frame_index, sizeof(int32_t));
//    // output_buffer += sizeof(int32_t);
...
//    // output_buffer += sizeof(int32_t);
//    //
//    // int32_t outScore = detect_only ? circleCount : last_score_out;
//    // std::memcpy(output_buffer, &outScore, sizeof(int32_t));
//    // output_buffer += sizeof(int32_t);
//    //
//    // std::vector<uchar> encoded;
//    // cv::imencode(".png", frame, encoded);
//    // int32_t frame_bytes = static_cast<int32_t>(encoded.size());
//    // std::memcpy(output_buffer, &frame_bytes, sizeof(int32_t));
//    // output_buffer += sizeof(int32_t);
//    // std::memcpy(output_buffer, encoded.data(), frame_bytes);
//    // mtx.unlock();
//    // }
//    //
...
//    //         frame,
//    //         scoreText,
//    //         cv::Point(50, 80),               // just below frame/fps text
//    // cv::FONT_HERSHEY_SIMPLEX,
//    // 0.8,                             // font scale
...
//    // int runningFps = cvRound(cv::getTickFrequency() / (t1 - t0));
//    //
//    // std::string text = "#frame " + std::to_string(frame_index)
//    //                    + " fps: " + std::to_string(runningFps);
...
//    // cv::imencode(".png", frame, encoded_image_bytes);
//    //
//    // int32_t frame_bytes = static_cast<int32_t>(encoded_image_bytes.size());
//    // std::memcpy(output_buffer, &frame_bytes, sizeof(int32_t));
//    // output_buffer += sizeof(int32_t);
//    // std::memcpy(output_buffer, encoded_image_bytes.data(), frame_bytes);
//    // mtx.unlock();
